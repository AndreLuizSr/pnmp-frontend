...
const InstitutionEdit = () => {
  ...
  useEffect(() => {
    axiosInstance
      ...
      .then((response) => {
        const unitsData = response.data.map((unit) => ({ code: unit.code, type: unit.type }));
        setUnits(unitsData);
      })
      .catch((error) => {
    ...
    axiosInstance
      .get(`http://localhost:3000/institutions/${_id}`)
      .then((response) => {
  ...
  const handleSubmit = (event) => {
    ...
    } else if (selectedUnit.type !== 'Cidade') {
      newErrors.unit = 'The unit must be a "City"';
    }
    if (type.length === 0) {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        axiosInstance
          .put(`http://localhost:3000/institutions/${_id}`, {
            code,
            name,
            phone,
            address: {
