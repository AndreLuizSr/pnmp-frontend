...
export default async function User(params: UserModel) {
  const session = await getServerSession(nextAuthOptions)
  
    return (
      <tr className="border-b"> 
              <td className='w-10 border border-slate-300'>{session?.user.name}</td>
              <td className='border border-slate-300'>{session?.user.email}</td>
              <td className='border border-slate-300'>{session?.user.phone}</td>
